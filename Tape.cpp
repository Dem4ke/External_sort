...
void Tape::moveLeft() {
	sleep((float)set_.getSetArray(1));
	--curCell_;
}
...
void Tape::write(int* ptr) {
	sleep((float)set_.getSetArray(0));
	*ptr = temp_;
}
...
void Tape::readTape(std::ifstream& input) {
	while (input >> temp_) {
    	sleep((float)set_.getSetArray(2)); // read
    	sleep((float)set_.getSetArray(0)); // write
		sleep((float)set_.getSetArray(1)); // move
...
void Tape::sortTape() {
	...
	while (length != 0) {
        ...
        for (int i = 0; i < curTape_.size() - 1; ++i) {
            ...
            if (temp_ > *curCell_) {
				sleep((float)set_.getSetArray(2));	
                temp1_ = *curCell_;
				moveLeft();
                sleep((float)set_.getSetArray(2));
				temp2_ = *curCell_;
